...
async def show_stats(cb: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        "<b>Месяц:</b>\n"
        f"Заказов: {data['month']['count']}\n"
        f"Выручка: {int(data['month']['revenue'])} ₽\n\n"
        "<b>Статусы (на текущий день):</b>\n"
        f"Активные: {data['statuses']['active']}\n"
        f"Завершённые: {data['statuses']['closed']}\n"
        f"Отменённые: {data['statuses']['cancelled']}\n\n"
